...
UiUtils::SignalNameToString(int32 signal, BString& _output)
{
	...
	switch (signal) {
		DEFINE_SIGNAL_STRING(HUP);
		DEFINE_SIGNAL_STRING(INT);
		DEFINE_SIGNAL_STRING(QUIT);
		DEFINE_SIGNAL_STRING(ILL);
		DEFINE_SIGNAL_STRING(CHLD);
		DEFINE_SIGNAL_STRING(ABRT);
		DEFINE_SIGNAL_STRING(PIPE);
		DEFINE_SIGNAL_STRING(FPE);
		DEFINE_SIGNAL_STRING(KILL);
		DEFINE_SIGNAL_STRING(STOP);
		DEFINE_SIGNAL_STRING(SEGV);
		DEFINE_SIGNAL_STRING(CONT);
		DEFINE_SIGNAL_STRING(TSTP);
		DEFINE_SIGNAL_STRING(ALRM);
		DEFINE_SIGNAL_STRING(TERM);
		DEFINE_SIGNAL_STRING(TTIN);
		DEFINE_SIGNAL_STRING(TTOU);
		DEFINE_SIGNAL_STRING(USR1);
		DEFINE_SIGNAL_STRING(USR2);
		DEFINE_SIGNAL_STRING(WINCH);
		DEFINE_SIGNAL_STRING(KILLTHR);
		DEFINE_SIGNAL_STRING(TRAP);
		DEFINE_SIGNAL_STRING(POLL);
		DEFINE_SIGNAL_STRING(PROF);
		DEFINE_SIGNAL_STRING(SYS);
		DEFINE_SIGNAL_STRING(URG);
		DEFINE_SIGNAL_STRING(VTALRM);
		DEFINE_SIGNAL_STRING(XCPU);
		DEFINE_SIGNAL_STRING(XFSZ);
		DEFINE_SIGNAL_STRING(BUS);
		default:
			break;
	}

	if (signal >= SIGNAL_REALTIME_MIN
			&& signal <= SIGNAL_REALTIME_MAX) {
		// realtime signal
		_output.SetToFormat("SIGRT%" B_PRId32, signal - SIGNAL_REALTIME_MIN);
	} else {
		// unknown signal
		_output.SetToFormat("SIG#%" B_PRId32, signal - SIGNAL_UNKNOWN_BASE);
	}
